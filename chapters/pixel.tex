...
\begin{figure}
  %\includegraphics[width=1\linewidth]{figures/pixel/image_corruption_x_50.pdf}
...
%TODO can we plot the activation map of soft vs hard threshold to show this?
%TODO define what it means to 'recover adversarial image'
%TODO should we give intuition about wavelet transform as using different size brushes to paint different objects (originally by Ingrid Daubechies
...
%In section ~\ref{sec:results} we have compared the results. 
...
%TODO, Can the sentence above be written in a better way?
...
%    \SetKwInOut{Output}{Output}
    %\underline{function pixel-update} $(image)$\;
%    \Input{Image of height $H$, width $W$ and channel $C$}
%    \Output{Image of same dimensions}
%    	   \For{$i\gets0$ \KwTo $K$}{
%           $\text{rnd}_h$, $\text{rnd}_w$  = $\mathcal{U}(0,H)$, $\mathcal{U}(0,W)$ \\
%           \Do{ $0 \leq$ $\text{ngh}_h$, $\text{ngh}_w$  $\leq$ H, W} { %TODO: repicking if you land outside the image is confusing.  Simplify?  Maybe take the intersection of R and the image, and say you pick uniformly from that?
%           		$\text{ngh}_h$, $\text{ngh}_w$  = $\mathcal{U}$($\text{rnd}_w\pm r$), $\mathcal{U}$($\text{rnd}_h \pm r$) \\
%                }
%			\;
%           		\For{$c\gets0$ \KwTo $C$}{
%                    \textit{image}[$\text{rnd}_h$, $\text{rnd}_w$, $c$] = \textit{image}[$\text{ngh}_h$, $\text{ngh}_w$, $c$]
%                }
%    }
%    \caption{Robustness to pixel loss}
%\end{algorithm}
...
\begin{figure}
   ...
   \caption{Average classification probabilities for an adversarial image (top) and clean image (bottom) after pixel deflection (Image size: 299x299)}
   \label{fig:pixelloss}
\end{figure}
...
%TODO add what is the difference between L_infinity and L_2 attacks here, 
% are both equally good/bad.
...
\begin{figure}
   ...
   \caption{Visualization showing average location in the image where perturbation is added by an attacker.
...
%TODO some form of empirical validation here will be useful
%TODO: does it need clarification on why this is bayesian approach?
...
\begin{figure}
   ...
   \caption{Difference between standard activation maps and robust maps under the presence of an adversary.}
   \label{fig:robustcam}
\end{figure}
...
%While class activation maps (CAM) are a function of an image and a given class, robust activation maps are a function of a given image alone.
...
\begin{figure}
   ...
   \caption{Linear search for model parameters}
...
\end{tabular}
%\vspace{-3mm}
\caption{Top-1 accuracy of our model on various attack models.}
%\vspace{-2mm}
\end{table}
...
\textbf{C\&W} & 0.04 & 04.8 & 92.7 & \textbf{98.0} \\
\end{tabular}
\caption{Top-1 accuracy of ResNet-50 with pixel deflection on various attack models. 
 \label{tblmodels}}
\end{table}
...
\end{tabular}
\caption{Top-1 accuracy of VGG-19 with pixel deflection on various attack models. 
 \label{tblmodels1}}
\end{table}
...
%Only known transformation based defenses against large scale images are currently under review ~\cite{MitigatingAnon208,CounteringAIGuo17}.
%Second, because the targeted $\boldmath{L}_2$ value varies across different attacks, thus adversary is not at same strength.
...
\centering
   ...
   %\caption{Overlap of classes between Adversarial and Original Image}
...
%TODO: why do we say this? it looks like they did quite well prior to our defense
%Reported results on C\&W are for confidence of $0.1$
%TODO should we say that this is the reason why in Figure 4 second column is at 0?
...
\end{tabular}
\caption{ Params: $\sigma=0.04$, Window=10, Deflections=100 \\
...
\end{tabular}
%\vspace{-3mm}
\caption{Top-1 accuracy with different deflections.}
%\vspace{-3mm}
\end{table}
...
\end{tabular}
%\vspace{-3mm}
\caption{Top-1 accuracy with different window sizes.}
%\vspace{-4mm}
\end{table}
...
\end{tabular}
%\vspace{-3mm}
\caption{Top-1 accuracy averaged across all six attacks.}
%\vspace{-4mm}
\end{table}
