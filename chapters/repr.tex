...
%We designed our training scheme for specially convolutional filters and we show that it is important to allow each of the model (with reduced filters) to converge before switching over. %TODO: move note about allowing each model to converge elsewhere
...
\subsection{Architecture Search } %It is possible to argue that works neural architecture search ~\cite{Liu2017ProgressiveNA, Real2018RegularizedEF, Zoph2016NeuralAS} is related to our technique. 
%Neural architecture search ~\cite{Liu2017ProgressiveNA, Real2018RegularizedEF, Zoph2016NeuralAS} (NAS) finds optimal network structure given a dataset.
%They iterate over large possible space of networks and generally yield state-of-the-art models.
%while our training regime restricts itself to the original form. 
...
%Our experiments show that explicit regularization by adding weight orthogonality term to the loss function does not promote a multitude of directions captured by the features.
...
\begin{figure}[H]
   ...
   %%\vspace{-2mm}
   \caption{Left: Canonical Correlation Analysis of activations from two layers of a ConvNet trained on CIFAR-10. Right: Distribution of change in accuracy when the model is evaluated by dropping one filter at a time.}
   \label{fig:CCA}
   %\vspace{-2mm}
\end{figure}

We will consider a simple network with two convolutional layers of $32$ filters each, and a softmax layer at the end.
%Consider a two-layer convolutional network with $32$ filters each and a softmax layer at the end.
%This model, when trained on CIFAR-$10$ with SGD for $100$ epochs with annealed learning rate, achieves test accuracy of $58.2$\%, which is far below VGG-$16$'s accuracy of $93.5$\%.
...
%TODO adds about how most weights/filters are useless and only a few of them are doing the task.
...
\begin{figure}[H]
    ...
    \caption{Left: Pearson correlation coefficient of various metric values with the accuracy values from greedy Oracle.
...
\begin{tabular}{rl}
    ...
    \end{tabular}
    \caption{Test accuracy on CIFAR-10 using standard training and RePr training with various metrics}
...
\begin{figure}[H]
   ...
   %%\vspace{-2mm}
   \caption{Left: Impact of using various optimizers on RePr training scheme. Right: Results from using different S1/S2 values. For clarity, these experiments only shows results with $S1=S2$}
...
\center
   ...
   %\vspace{-3mm}
   \caption{Test accuracy of a three layer ConvNet with 32 filters each over $100$ epochs using \textcolor{orange}{standard scheme} and \textcolor{blue}{our method - RePr} on CIFAR-$10$. The shaded regions denote periods when only part of the network is trained for RePr.
...
\center
   ...
   %\vspace{-3mm}
   \caption{Test accuracy of a three layer ConvNet with 32 filters each over $100$ epochs using \textcolor{red}{standard scheme}, \textcolor{blue}{RePr with Oracle} and \textcolor{green}{RePr with Ortho} on CIFAR-$10$. Left: With Dropout of $0.5$. Right: No Dropout}
   \label{fig:dropout}
   %%\vspace{1mm}
\end{figure}

\subsection{Orthogonal Loss - OL}
...
\begin{figure}[H]
   ...
   %%\vspace{-2mm}
   \caption{Comparison of orthogonality of filters (Ortho-sum - eq 2) in standard training and RePr training with and without Knowledge Distillation. \textbf{Lower value} signifies less overlapping filters.
...
\end{tabular}
%%\vspace{-2mm}
\caption{Comparison of Knowledge Distillation with RePr.}
...
\center
   ...
   %%\vspace{-2mm}
   \caption{Accuracy improvement using RePr over standard training on Vanilla ConvNets across many layered networks [$C^n(32)$]}
...
\bottomrule
\end{tabular}
    \caption{Comparison of test error from using various techniques. }
   \label{tbl:resnet20}
   %%\vspace{-2mm}
\end{table}
...
%For object detection on COCO~\cite{Lin2014MicrosoftCC}, using Feature Pyramid Network~\cite{Lin2017FeaturePN} and ResNet-50, mAP improves from $38.2$ to $42.3$.
%For visual question answering on VQAv1, using VQA-LSTM-CNN model~\cite{Antol2015VQAVQ}, accuracy on Open-Ended questions increases from $60.3\%$ to $64.6\%$. 
...
\end{tabular}
\caption{Sample of differences in confidence of selected answers between Standard Trainin and RePr training}
